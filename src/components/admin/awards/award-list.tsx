...
function AwardCard({
  ...
  const dateInfo = award.date ? `Date: ${new Date(award.date).toLocaleDateString()}` : null;

  return (
    <div className="rounded-(--border-radius) border border-border bg-card overflow-hidden text-card-foreground">
      {/* Card header */}
      <div className="flex items-start justify-between gap-4 px-5 py-4">
        <div className="min-w-0 flex-1 space-y-0.5">
          ...
          <p className="text-sm text-foreground truncate">
            {award.issuer || 'No issuer specified'}
          </p>
          {dateInfo && (
            <p className="text-xs text-muted-foreground pt-0.5">{dateInfo}</p>
          )}
          {award.description && (
            <p className="text-sm text-muted-foreground line-clamp-2 mt-2">{award.description}</p>
          )}
          {award.url && (
            <a 
              href={award.url} 
...
export function AwardList({ awards: initialAwards }: AwardListProps) {
  const router = useRouter();
  const [awardItems, setAwardItems] = useState<AwardRow[]>(initialAwards);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [showAddForm, setShowAddForm] = useState(false);

  // Sync state when props change
  useEffect(() => {
    setAwardItems(initialAwards);
  }, [initialAwards]);

  function handleDeleted(id: string) {
    setAwardItems((prev) => prev.filter((a) => a.id !== id));
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          {awardItems.length === 0
            ...
            : `${awardItems.length} award${awardItems.length === 1 ? '' : 's'}`}
        </p>
        {!showAddForm && (
          <button
            type="button"
      ...
      {showAddForm && (
        <div className="rounded-(--border-radius) border border-border bg-card px-5 py-5">
          <h3 className="mb-4 text-sm font-semibold text-card-foreground">New Award or Achievement</h3>
          <AwardForm
      ...
      {/* Award cards */}
      {awardItems.length === 0 && !showAddForm ? (
        ...
        <ul className="space-y-3" role="list">
          {awardItems.map((award) => (
            <li key={award.id}>
              <AwardCard
                award={award}
                isEditing={editingId === award.id}
                onEdit={() => {
                  setEditingId(award.id);
                  setShowAddForm(false);
                }}
                onCancelEdit={() => setEditingId(null)}
                onDeleted={() => handleDeleted(award.id)}
              />
            </li>
          ))}
        </ul>
